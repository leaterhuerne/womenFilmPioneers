...
const SidePanel = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  let { locations = new Array(5).fill("") } = $$props;
  ...
  let { refreshFilms = () => data.getFilmsForYear(year, (json) => {
    locations = [];
    Object.keys(json).filter((film) => country != "alle" ? json[film].location.includes(country) : true).sort(() => 0.5 - Math.random()).slice(0, 5).forEach((film) => locations.push(json[film]["title"]));
  }) } = $$props;
  ...
  return `<div class="grow lg:w-1/3 border-t lg:border-t-0 lg:border-l border-firebrick-500 dark:border-firebrick-1000 p-2 flex flex-col gap-2 "><h1 class="text-3xl font-semibold text-center">${validate_component(T, "T").$$render(
    ...
    {}
            ...
            <button>${validate_component(Refresh, "Refresh").$$render($$result, { darkColor: "#D2CAB3" }, {}, {})}</button></div>
        <ul class="list-disc ml-4">${each(locations, (location) => {
    return `<li>${escape(location)}</li>`;
  })}</ul></div></div>`;
});
...
//# sourceMappingURL=_page.svelte-e24b1c6a.js.map
